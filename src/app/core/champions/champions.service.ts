...
export class ChampionsService {
    ...
    getChampions(): Observable<any> {
      return this.getLeagueOfLegendsVersions().pipe(
            switchMap((versions: string[]) =>
                this.http.get<any>(
                  `${this.baseUrl}/${versions[0]}/${this.pathEnglishLanguage}.json`
                )
            )
        );
    }
  ...
  getSplashChampion(championId: string): Observable<any> {
    return this.http.get(`https://ddragon.leagueoflegends.com/cdn/img/champion/loading/${championId}_0.jpg`);
  }
}
